...
function createXMLHttpRequest() {
    ...
    try {
        ...
        // override open function to add custom header
        xhr.open = function() {
            xhr._method = arguments[0];
            xhr._url = arguments[1];
            if (arguments.length > 2) {
                xhr._async = arguments[2];
            } else {
                xhr._async = true;
            }
            ...
            // add specific header
            //--------------------
            this.setRequestHeader("Authentication-Info", sessionStorage.getItem("authentication_info"));
            return this;
        };
        return xhr;
...
function formatLastApplicationError(val) {
    if (val == 65535) return "Unknown (65535)";
    ...
    if (val == 20001) return "Mismatch in the application version detected (" + val + ")";
    return val;
}
...
function formatExtensionProductID(val) {
    ...
    if (val == 51430) return "TM3XFHSC202 (" + val + ")<br/>High Speed Counting Event Management 200kHz 2 channels";
    if (val == 51431) return "TM3XPTO102 (" + val + ")<br/>Pulse Train Output 100kHz 2 channels";
    if (val == 51433) return "TM3SAK6R (" + val + ")<br/>Safety module, 3 functions, CAT4, maximum PL e/SIL3";
    if (val == 51434) return "TM3SAFL5R (" + val + ")<br/>Safety module, 2 functions, CAT3, maximum PL d/SIL2";
    if (val == 51435) return "TM3SAF5R (" + val + ")<br/>Safety module, 1 function, CAT4, maximum PL e/SIL3";
    if (val == 51436) return "TM3SAC5R (" + val + ")<br/>Safety module, 1 function, CAT3, maximum PL d/SIL2";
    if (val == 51440) return "TM3XHSC202 (" + val + ")<br/>High Speed Counting 200kHz 2 channels";
	if (val == 0) return "-";
...
var SYS_AREA = [
    ...
    ['ETH_R.MACAddress', 6, false, formatMacAddress],
    ...
    ['ETH_R.MACAddress_If2', 6, false, formatMacAddress],
...
function userSite(){return false;}
